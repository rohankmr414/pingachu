...
export class DB {
  ...
  async createRole(name: string, chatId: number): Promise<Role> {
    const result = await this.DB.prepare("INSERT INTO roles (name, chat_id) VALUES (?, ?)")
      .bind(name, chatId)
      .run();
  ...
  async listRoles(chatId: number): Promise<Role[]> {
    const rows = await this.DB.prepare("SELECT id, name, chat_id FROM roles WHERE chat_id = ?")
      .bind(chatId)
      .all();
  ...
  async addRoleToUserByUsername(roleId: number, username: string): Promise<void> {
    await this.DB.prepare("INSERT INTO role_assignments (role_id, username) VALUES (?, ?)")
      .bind(roleId, username)
      .run();
  }

  async removeRoleFromUserByUsername(roleId: number, username: string): Promise<void> {
    await this.DB.prepare("DELETE FROM role_assignments WHERE role_id = ? AND username = ?")
      .bind(roleId, username)
      .run();
  }

  async getRoleMemberUsernames(roleId: number): Promise<string[]> {
    const rows = await this.DB.prepare("SELECT username FROM role_assignments WHERE role_id = ?")
      .bind(roleId)
      .all();
